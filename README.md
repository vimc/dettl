...
`dettl` requires a log table to exist in the target database for an import to be run. Please see `inst/sql/create_log_table.sql` for a query to create the log table. The name of the table can be changed and configured via the import `dettl_config.yml` but the fields must remain the same.
