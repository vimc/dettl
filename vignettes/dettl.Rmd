...
The final stage to implement is the load stage. Note that you can use an automatic load stage. This will append the `transformed_data` to existing tables in the database respecting any foreign key constraints configured via the `dettl.yml`. See section below for details about [automatic load](#dettl_auto_load). To implement this open the `R/load.R` file. An automatically generated implementation will already be there for you, this simply takes each data frame in the transformed data list and appends those rows to the relevant table in the database. If something more complicated is required this can be replaced. Expect that this stage is only really a set of update queries to be run on the database implemented in R.
...
This returns a list containing the import object which you can work further with and returns a list of processed data. You can see the extracted data at

```{r}
import$data$extract
```

To execute the transform stage, call the same run function with `stage = "transform"`. This returns the same list as from the extract stage. You can call the function either using the path to import and the db to import to or using the object returned from extract run.

```{r}
import <- dettl::dettl_run(import, stage = "transform")
```
...
You can run the load stage using the returned import object.

```{r}
dettl::dettl_run(import, stage = "load")
```
...
## Automatic load stage {#dettl_auto_load}

`dettl` has an automatic implementation for the load stage of the import. At its most simple this will just take the named list of `transformed_data` and append each of these data frames to the table in the database with matching name. To use the automatic load step requires defining that the automatic load should be used in the load block of `dettl.yml` e.g.
...
Using an automatic load means that the configured `sources` in the `dettl.yml` no longer need to define a `load` function. A file containing tests for the load stage is still required.
