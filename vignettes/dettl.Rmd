...
Dettl provides a way to make data tidying and ingestion into a database easier, more testable and more robust. We do that by employing an ETL (extract, transform, load) like workflow to separate out different concerns of your data import process and allow testing after each stage of the process.
...
* `dettl_config.yml` - Configuration at the project level used to configure DB connection information for any database which you wish to import to for each ETL process subdirectory. Note that you must specify the `log_table` for each database. This is the table which import runs will be logged to.
...
If you want to create a new process within an existing project this can be done by opening the project as an RStudio project and using `new_dettl()`. Note that the use of `withr::with_dir(tempdir(), ...)` in the example below is because this is running in a vignette and the code is an example. This is not necessary when using `dettl` in a real environment.
...
If you want to create a new process within a new project then you will need to manually create a new RStudio project and create a `dettl_config.yml` file. You can then proceed as above.
...
After completing the implementation of the extract stage you can run just that step to check that the extracted data looks as expected. Note that the directory must be under version control and up to date with remote to run the import process. This check is skipped if running in `dry_run` mode. See [Running the process](#run) for more details about this. When creating a new import also pass the database which this import is for, this must match one of the databases configured in the `dettl_config.yml`. See example below for a previously configured example. Note that the preparation step which calls `prepare_test_import` is only used to setup the import in a temporary directory as this is a vignette. This won't be needed in a real world scenario.
...
dettl::run_import(import, "extract")
...
```{r}
dettl::run_import(import, "transform")
...
After this stage has been implemeneted the import process can be run end-to-end.

## Running the process {#run}

The process can be run by first creating a new import object using the name of the directory containing the process you want to run. Again note that the `prepare_test_import` call here is only needed because this is being run within a vignette and is not required in a real world scenario.

```{r}
path <- dettl:::prepare_test_import(
  system.file("examples", "person_information", package = "dettl"),
  system.file("examples", "dettl_config.yml", package = "dettl"))
import <- dettl::dettl(file.path(path, "person_information"), 
                       db_name = "test")
```
...
```{r}
dettl::run_import(import, dry_run = TRUE)
```
...
`dettl` has a default implementation for the load stage of the import process. At its most simple this will just take the named list of `transformed_data` and append each of these data frames to the table in the database with matching name. To use the default load step requires defining that the default load should be used in the load block of `dettl.yml` e.g.
