...
```
dettl::new_dettl("person_information")
```
...
After completing the implementation of the extract stage you can run just that step to check that the extracted data looks as expected. See [Running the process](#run) for more details about this.

```
import <- dettl::new_import("person_information")
...
```
import <- dettl::new_import("person_information")
import <- dettl::run_import(import, c("extract", "transform"))
