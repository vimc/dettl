...
* `db_config.yml` - Configuration at the project level used to configure DB connection information for any database which you wish to import to for each ETL process subdirectory. e.g.
...
After completing the implementation of the extract stage you can run just that step to check that the extracted data looks as expected. See [Running the process](#run) for more details about this. When creating a new import also pass the database which this import is for, this must match one fo the databases configured in the `db_config.yml`. See example below for a previously configured example.
...
`dettl` has a default implementation for the load stage of the import process. At its most simple this will just take the named list of `transformed_data` and append each of these data frames to the table in the database with matching name. To use the default load step initialise the import as 
```{r}
import <- dettl::dettl(import_path, default_load = TRUE, db_name = "test")
```

Using a default_load means that the configured `sources` in the `dettl.yml` no longer need to define a `load` function. A file containing tests for the load stage is still required.
