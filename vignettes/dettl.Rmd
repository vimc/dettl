...
```{r}
import <- dettl::run_import(import, "transform", dry_run= TRUE)
...
db_name2 <- "test2.sqlite"
db_path2 <- dettl:::prepare_example_db(db_name2, 
  system.file("examples", package = "dettl"))
on.exit(unlink(db_path2), add = TRUE)
```
```{r}
import_path <- system.file("examples", "person_information", package = "dettl")
import <- dettl::dettl(import_path, db_name = "test")
...
where `id` is the primary key of table `people` which is used as a foreign key for the `person` column of table. `jobs`. In the transformed data these `id`s are just temporary values used to associate the new rows of data with each other. The tables themselves may already have entries with these primary keys used so when the default load is run the primary keys are updated when a new record is added. 

We need to configure in the `dettl.yml` which fields in which tables are primary keys and where they are used as foreign keys. For example data above this is done as follows
